...
/* With algo -	RSA (Public Key/Private Key pair) */
	...
	if(checkAccount($acc,$pwd,$conn)){
		...
		echo $_COOKIE['token'];
		header("Location:http://127.0.0.1/bootstrap-WSSE/index.html");
	}else{
		$failToLogin = $_COOKIE["failToLogin"];
		$failToLogin += 1;
		setcookie("failToLogin",$failToLogin,time()+(300),"/","127.0.0.1",0,0);
		header("Location:http://127.0.0.1/bootstrap-WSSE/login.html");
	...
	function packageToken($acc){
		...
		setcookie("token",$token,time()+(3600),"/","127.0.0.1",0,0);
		echo $token;
	}
